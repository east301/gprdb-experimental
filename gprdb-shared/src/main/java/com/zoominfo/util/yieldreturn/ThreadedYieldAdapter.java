...
public class ThreadedYieldAdapter<T> implements YieldAdapter<T> {
     ...
     * This means that the collecting method can use complex recursive logic, but still allows
     * the calling code to handle the results with a standard iterator. Results are returned immediately
     * and do not incur overhead of being stored in a list. Calculation overhead is only performed for
     * the results that are requested through the iterator.
     *
